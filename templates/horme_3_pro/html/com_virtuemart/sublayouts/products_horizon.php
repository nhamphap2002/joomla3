            ...
            <?php echo shopFunctionsF::renderVmSubLayout('badges',array('product'=>$product)); // Product Badges ?>
		  			<div class="vm-product-media-container" style="min-height: <?php echo VmConfig::get ('img_height'); ?>px">
		    			<a title="<?php echo $product->product_name ?>" href="<?php echo $product->link.$ItemidStr; ?>">
		  				<?php
		            if (!$product->images[0]->getFileUrlThumb() || !$lazy) {
		              echo $product->images[0]->displayMediaThumb('class="browseProductImage"', false);
		            } else {
		              echo '<img class="browseProductImage lazy" data-original="'.JURI::root().$product->images[0]->getFileUrlThumb().'" alt="'.$product->product_name.'"/>';
		            } ?>
		    			</a>
		  			</div>
		      </div>
