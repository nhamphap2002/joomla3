...
$bodyclass = $home . ' ' . $option . ' fds_' . $view . ' ' . $style;
...
// Logo size
$logosize = $this->params->get('headersize');
switch ($logosize) {
  case '4':
      $logosize = '4';
      $searchsize = '4';
...
switch ($style) {
	case 'default' :
		$css = '/css/default.bootstrap.min.css';
		break;
	default :
    $css = '/css/bootstrap.min.css';
}
$doc->addStyleSheet($tpath . $css);
$doc->addStyleSheet(''.$tpath.'/css/template.css');

// The css file that styles the template
if ($this->params->get('style') != 'none') {
...
// Google fonts
if ( $hfonts != 'default' && $bfonts == 'default' ){
  $doc->addStyleSheet('//fonts.googleapis.com/css?family='.$hfonts.':300,400,700&amp;subset=latin,greek,cyrillic');
} elseif ($hfonts == 'default' && $bfonts != 'default' ) {
  $doc->addStyleSheet('//fonts.googleapis.com/css?family='.$bfonts.':300,400,700&amp;subset=latin,greek,cyrillic');
} elseif ($hfonts != 'default' && $bfonts != 'default' && $hfonts == $bfonts ) {
  $doc->addStyleSheet('//fonts.googleapis.com/css?family='.$hfonts.':300,400,700&amp;subset=latin,greek,cyrillic');
} elseif ($hfonts != 'default' && $bfonts != 'default' && $hfonts != $bfonts) {
  $doc->addStyleSheet('//fonts.googleapis.com/css?family='.$hfonts.'|'.$bfonts.':300,400,700&amp;subset=latin,greek,cyrillic');
}
