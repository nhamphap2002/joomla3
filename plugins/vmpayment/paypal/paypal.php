    ...
    function plgVmOnProductDisplayPayment($product, &$productDisplay) {
        ...
        foreach ($this->methods as $this->_currentMethod) {
            if ($this->_currentMethod->paypalproduct == 'exp') {
                ...
                $productDisplayHtml = $this->renderByLayout('expproduct', array(
    ...
    function getExpressCheckoutHtml($cart) {
        ...
        foreach ($this->methods as $this->_currentMethod) {
            if ($this->_currentMethod->paypalproduct == 'exp') {
                ...
                $html .= $this->renderByLayout('expcheckout', array(
        ...
        if ($this->_currentMethod->paypalproduct == 'std') {
            ...
            if ($this->_currentMethod->paypalproduct == 'exp') {
                ...
                if ($success) {
                    ...
                    $returnValue = 1;
                    $html = $this->renderByLayout('expresponse', array("method" => $this->_currentMethod,
                        "success" => $success,
...
//			$this->customerData->clear();
    ...
    private function _loadPayPalInterface() {
        ...
        if (empty($this->_currentMethod->paypalproduct))
