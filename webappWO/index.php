...
//require_once './PhpTemplates/addProduct.php';
...
function addProduct($productIT, $productUK, $productFR, $productDE, $hash, $_ParentId, $childrenArrayIds) {
    ...
    if (!class_exists('ProductWebservice'))
        require('JsonTrial.php');
...
//        die();
    case 'editProductCompletelyWithChildren':
        try {
            // edit parent first
            $PartsToAdd = addProduct($dec["Parent"]['productIT'], $dec["Parent"]['productUK'], $dec["Parent"]['productFR'], $dec["Parent"]['productDE'], $hash, $dec["Parent"]['virtuemart_product_id']);
        } catch (Exception $r) {
            $_SESSION['Errors']['Errors'][] = 'Errore Di sistema : ' . $r->getMessage();
        }
        ...
        foreach ($productChildrenNumber as $i) {
            ...
            try {
                ...
                if ($dec["Children"][$i]['productIT']['product_in_stock'] == 0 && $dec["Children"][$i]['productIT']['published'] == 0) {
                    ...
                    if ($a->unpublishProduct($dataChUnpls))
                        $product_published = 0;
                } else {
//                    $PartsToAdd=addProduct($productIT, $productUK, $productFR,$productDE, $hash);
                    $ChildrenPartsToAdd = addProduct($dec["Children"][$i]['productIT'], $dec["Children"][$i]['productUK'], $dec["Children"][$i]['productFR'], $dec["Children"][$i]['productDE'], $hash, $_ParentId, null);
                    $product_published = 1;
                }
            } catch (Exception $r) {
                $_SESSION['Errors']['Errors'][] = 'Errore Di sistema : ' . $r->getMessage();
...
//            $productIT[$hash] =1;
        ...
        try {
            $PartsToAdd = addProduct($dec["Parent"]['productIT'], $dec["Parent"]['productUK'], $dec["Parent"]['productFR'], $dec["Parent"]['productDE'], $hash, null, null);
        } catch (Exception $r) {
            $_SESSION['Errors']['Errors'][] = 'Errore Di sistema : ' . $r->getMessage();
        }
...
//echo ' provo con file senza nulla.'.$file;
    ...
    if (isset($ChildrenArray) && count($ChildrenArray) > 0) {
        ...
        if (printOnfile(json_encode($localDatas), $filetto))
            return true;
    }
